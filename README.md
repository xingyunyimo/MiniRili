...
| 功能模块 | 状态 |
|---|---|
| 万年历（公历 + 农历 + 节气，月/日/年视图，周视图代码已内置但入口暂未开放） | ✅ 已完成 |
| 事件管理与一次性提醒（全天事件当天 09:00 提醒） | ✅ 已完成 |
| 周期性重复提醒（每日/每周/每月/每年/工作日/周末 + 农历每月/农历每年） | ✅ 已完成 |
| 提醒防漏机制（开机重排、被杀恢复、锁屏停闹钟、跳过当天后可恢复） | ✅ 已完成 |
| 导入栏 + 通知双通道提醒 | ✅ 已完成 |
| 本地导入（ICS / JSON）、导出（ICS） | ✅ 已完成 |
| 搜索（标题 + 描述） | ✅ 已完成 |
| 节假日标记（含调休） | ✅ 已完成 |
| 标签分类与颜色标记 | ✅ 已完成 |
| 周期事件例外（跳过某次触发） | ✅ 已完成 |
| 天气栏目与详情页（Open-Meteo + 定位 + Room 缓存） | ✅ 已完成 |
| 出行建议规则引擎（高温/降雨/大风/温差等场景化提醒） | ✅ 已完成 |
| 每日天气通知（AlarmManager 定时推送 + 可配置时间） | ✅ 已完成 |
...
- **城市数据库**：内置全国县级及以上行政区约 3320 条经纬度数据（`ChineseCityDb`）
...
│   │   │   │   ├── ui/navigation/  # 导航配置（Screen sealed class）
│   │   │   │   ├── data/           # 数据层（天气/节假日）
│   │   │   │   ├── database/       # Room 数据库
│   │   │   │   ├── repository/     # 仓库层
│   │   │   │   ├── scheduler/      # 提醒调度
